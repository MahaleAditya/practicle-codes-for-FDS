...
int diff1(struct node * , struct node *);
...
int SEA::diff1(struct node *p , struct node *q)
{
int  point=0;
while(p!=NULL)
{
...
break;
case 7:
first=a.diff1(head , head1);
second=a.diff1(head1 , head);
total=first+second;
cout<<"\n Students who not like neither Vanilla nor Butterscotch\n"<<total<<endl;
break; 
case 8:
cout<<"Students who not like either Vanilla or Butterscotch but not both\n"<<endl;
a.diff1(head , head1);
a.diff1(head1 , head);
break; 
}
